...
import { PenLine, Loader2, ArrowRightIcon } from "lucide-react";
...
function Landing() {
  ...
  const handleGetExtensionClick = () => {
    ...
    window.open(
      "https://chromewebstore.google.com/detail/intent/bmlpllpcbgbifinbbkfegnenmmnhakpf",
      "_blank"
    );
  };
    ...
    <div className="min-h-screen bg-background relative">
      <div className="fixed top-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-md z-50 flex items-center justify-between px-8">
        ...
        <div className="hidden sm:flex items-center gap-6 text-sm text-muted-foreground">
          ...
          <button
            ...
            ref={headerCtaRef}
            onClickCapture={() =>
              console.log("[Intent] Header Get Extension onClickCapture")
            }
      ...
      <div className="pt-40 sm:pt-44 px-6 max-w-[1200px] mx-auto">
        ...
        <p className="text-lg sm:text-2xl text-muted-foreground max-w-3xl">
          Intent keeps you grounded by asking for your purpose before accessing
          any distracting website.
        </p>
      ...
      <div className="max-w-[1200px] mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-10 gap-y-12 mt-12 sm:mt-16 justify-items-center">
          <Feature
            renderIcon={(ref) => (
              <SquarePenIcon ref={ref as Ref<any>} size={56} />
            )}
            title="Set your intention"
          />
          <Feature
            renderIcon={(ref) => (
              <ShieldCheckIcon ref={ref as Ref<any>} size={56} />
            )}
            title="Stay focused"
          />
          <Feature
      ...
      <div ref={bottomRef} className="text-center py-20 px-6">
        <p className="text-xl sm:text-2xl md:text-3xl font-semibold text-primary max-w-3xl mx-auto">
          ...
          <span className="block mt-2 text-muted-foreground font-normal text-lg sm:text-xl">
        ...
        <div className="mt-8 max-w-2xl mx-auto">
          <button
            ...
            className="group relative inline-flex items-center justify-center overflow-hidden bg-orange-500 text-white px-8 py-4 rounded-xl text-lg sm:text-xl font-semibold transition-all duration-500 hover:bg-orange-600 shadow-lg shadow-orange-500/30 hover:shadow-xl hover:shadow-orange-500/40"
          >
            <span className="transition-all duration-300 group-hover:-translate-x-48 group-hover:opacity-0">
              Get Extension Now
            </span>
            <span className="absolute inset-0 flex items-center justify-center transition-all duration-300 translate-x-48 opacity-0 group-hover:translate-x-0 group-hover:opacity-100">
        ...
        <div className="text-muted-foreground text-sm pb-10">
          ...
          <a href="/privacy" className="underline hover:text-primary ml-2">
            Privacy
          </a>
        </div>
      </div>
...
function Feature({
  ...
  renderIcon: (ref: Ref<IconHandle>) => ReactNode;
  title: string;
}) {
    ...
    <div className="bg-primary/80 backdrop-blur-lg rounded-xl md:rounded-4xl shadow-2xl overflow-hidden relative h-[480px] sm:h-[520px] md:h-[560px] flex items-center justify-center">
      ...
      >
        <div className="text-center opacity-70">
          ...
          <p className="text-lg">
            This represents a potentially distracting website
          </p>
        </div>
      </div>
